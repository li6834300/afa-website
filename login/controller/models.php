...
class models extends Admin {
    ...
    public function __construct() {
		parent::__construct();
		$this->model_prefix = array(1 => 'content',2 => 'member',3 => 'form',4 => 'diy',5 => 'category');
		$this->typename  = array(1 => '内容模型',2 => '会员模型',3 => '表单模型',4=> '自定义表',5=> '系统模型');
	    $this->typeid = (int)Request::get('typeid') ? (int)Request::get('typeid') : 1;
	...
	public function addAction() {
	    if (Request::isPost()) {
	        ...
	        $data['listtpl']      = Request::post('listtpl')     ? Request::post('listtpl') : 'list_' . $data['tablename'] . '.html';
	        $data['showtpl']  = Request::post('showtpl') ? Request::post('showtpl') : ($this->typeid == 3 ? 'form.html' : 'show_' . $data['tablename'] . '.html') ;
...
{/xiao:list}';
		$form_url  = SITE_PATH . 'index.php?c=index&a=form&modelid=' . $modelid . 'cid=$id ($id是被关联内容的id变量)';
        }
        ...
        $list_code = '
{xiao:list table=' . $data['tablename'] . '   num=10}
在这里可以调用你的diy字段信息 例如：id：{xiao:$xiao[\'id\']} 更多信息请参考官方模板帮助文档
{/xiao:list}';
	    }
	...
	public function fieldAction() {
	    ...
	    if (Request::post('listorder')) {
     		foreach (Request::post('listorder') as $fieldid => $value) {
	...
	public function addfieldAction() {
	    $modelid    = (int)Request::get('modelid');
	    if($modelid==0){
	    	$model_data['tablename']='category';
	    	$model_data['modelname'] = '栏目扩展';
	    }else{
	    ...
	    if (Request::isPost()) {
			...
			if(in_array($field,array('id','url'))) $this->show_message('请更换字段名称',2,1);
	        ...
	        $fields     = array_unique($fields+$core_fields);
			}
            ...
            if($modelid==0){
            	$this->show_message('添加成功', 1, url('models/field/', array('modelid'=>0, 'category'=>1, 'typeid'=>$this->typeid)));
            }else{
	...
	public function field_type_settingAction() {
	    ...
	    $field = new field();
	    if (!method_exists($field,$fieldsetting)) exit('');
		echo json_encode($field->$fieldsetting());
	}
	...
	public function delfieldAction() {
		...
		if ($data['modelid']=='0')
		{
			$this->db->execute("ALTER TABLE `#xiaocms_category` DROP `$data[field]`");
			}
	...
	public function cacheAction() {
		delete_dir(DATA_DIR . 'models' . DS);
		foreach ($this->model_prefix as $typeid=>$modeltype) {
			if($typeid==5){
				$fields = $this->db->setTableName('model_field')->getAll(array('modelid=0', 'disabled=0'),null,null,'listorder DESC');
				$_fields  = array();
				foreach ($fields as $k=>$f) {
					$_fields[$f['field']] = $f;
				}
				set_cache('category_fields', $_fields);
			}else{
