...
abstract class Base
{
    ...
    public function __construct()
    {
        ...
        if (is_file(XIAOCMS_PATH . 'member' . DS . 'index.php'))
		define('XIAOCMS_MEMBER', XIAOCMS_PATH . 'member' . DS);
    ...
    {
        include CORE_PATH . 'img' . DS . 'message' . DS . 'xiaocms_msg.tpl.php';
        exit;
    }
    ...
    {
        $code = Session::get('checkcode');
        Session::delete('checkcode');
    ...
    protected function get_member_info()
    {
        ...
        if (Cookie::get('member_id') && Cookie::get('member_code')) {
            $id = Cookie::get('member_id');
            $id = (int)$id;
    ...
    protected function get_data_fields($fields, $data = array())
    {
        ...
        foreach ($fields as $t) {
            ...
            if (method_exists($field, $t['formtype'])){
                $method = $t['formtype'];
                $data_fields .= $field->$method($t['field'], $content, $t['setting']);
            }
...
//			if (in_array($t['formtype'], array('checkbox', 'files', 'diy'))) $data[$t['field']] = array2string($data[$t['field']]);
    ...
    protected function getContent($id)
    {
        ...
        $category   = $this->category_cache[$data['catid']];
        $data2 = $this->db->setTableName($category['tablename'])->find($id);
        $data2 = $this->handle_fields($this->content_model[$data['modelid']]['fields'], $data2);
        $data  = $data2 ? array_merge($data,$data2) : $data;
        return $data;
    }
