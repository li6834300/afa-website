...
class URL{
    ...
    public function get_contents($url){
        ...
        //-------请求为空
        if(empty($response)){
            exit("可能未开启curl支持,请尝试开启curl支持，重启web服务器，如果问题仍未解决，请联系我们");
        }
...
class Oauth{
    ...
    function __construct(){
        $this->recorder = xiaocms::load_class('Recorder');
        $this->urlUtils = xiaocms::load_class('URL');
    }
...
class QC extends Oauth{
    ...
    public function __call($name,$arg){
        ...
        if(empty($this->APIMap[$name])){
            exit("api调用名称错误 不存在的API: <span style='color:red;'>$name</span>");
        }
