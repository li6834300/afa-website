...
class field
{
  ...
  <tr> 
      ...
      <td><input type="radio" value="0" name="data[setting][type]" ' . ($setting['type'] == 0 ? 'checked' : '') . ' onclick="$(\'#editor_diy\').hide();"> 百度编辑器&nbsp;&nbsp;&nbsp;&nbsp;
    <input type="radio" value="1" name="data[setting][type]"' . ($setting['type'] == 1 ? 'checked' : '') . ' onclick="$(\'#editor_diy\').hide();"> KindEditor
    </td>
  </tr>
  ...
  <tr  id="editor_diy"  ' . ($setting['toolbar'] != 2 ? 'style="display:none" ' : '') . ' > 
      <th>自定义配置：</th>
      <td><textarea name="data[setting][items]"  rows="5" cols="70" class="text">' . $setting['items'] . '</textarea><br/><font color="gray"><a href="#" class="green">点击这里查看配置帮助文档</a></font></td>
    </tr>
  ...
  <tr> 
	...
	<tr> 
      ...
      <td><textarea name="data[setting][content]" style="width:400px;height:120px;" class="text">' . $setting['content'] . '</textarea><br/><font color="gray">格式：名称|赋值 (一行一个)。<a href="#"  class="green">详见帮助文档</a></font>
      </td>
    </tr>
	<tr> 
      <th>默认选中值 ：</th>
	...
	<tr> 
      ...
      <td><textarea name="data[setting][form]" rows="2" cols="20"  style="height:150px;width:600px;">' . $setting['form'] . '</textarea><br/><font color="gray"><a href="#" class="green">具体设计请看官方帮助文档</a></font></td>
    </tr>
	<tr> 
      <th >是否数组：</th>
	...
	<tr> 
      <th>附加js/css：</th>
      <td><textarea name="data[setting][jscss]" rows="2" cols="20"  style="height:150px;width:600px;">' . $setting['jscss'] . '</textarea><br/><font color="gray"><a href="#" class="green">如果有js或css可在此添加，也可以在外部添加，没有可忽略，具体可以看帮助文档</a></font></td>
    </tr>
	</table>';
    }
    ...
    public static function editor($name, $content = '', $setting = '')
    {
      ...
      if($setting['type']){
        ...
        $other = defined('XIAOCMS_ADMIN') || defined('XIAOCMS_MEMBER') ? "allowFileManager : true,
        uploadJson : './index.php?c=uploadfile&a=kindeditor_upload', 
        fileManagerJson : './index.php?c=uploadfile&a=kindeditor_filemanager'," : "
        ...
        KindEditor.ready(function(K) {
          window.editor = K.create('#" . $name . "', { 
            " . $other . "items:[" . $source . "'forecolor','bold','italic','underline','lineheight','table','|','fontname','fontsize','code','plainpaste','wordpaste','|','image','multiimage','flash','media','insertfile','link','unlink','|','justifyleft','justifycenter','justifyright','justifyfull','insertorderedlist','insertunorderedlist','indent','outdent','clearhtml','quickformat','hr','baidumap'" . $page . ",'|','fullscreen']
          });
        });
      </script>";
    } else {
      ...
      KindEditor.ready(function(K) {
        window.editor = K.create('#" . $name . "', { 
          " . $other . "items:[" . $source . "'forecolor','bold','italic','underline','table','|','image','multiimage','flash','media','insertfile','link','unlink','fontname','fontsize','|','justifyleft','justifycenter','justifyright','justifyfull','clearhtml','quickformat','baidumap','|','fullscreen']
        });
      });</script>";
    }
  } else {

      $up = Base::get_base_url().'index.php?c=uploadfile&a=ueditor';

      if (!defined('XIAOCMS_EDITOR_U')) {
        $str .= '<script type="text/javascript" src="../core/img/ueditor/ueditor.config.js"></script>
        <script type="text/javascript" src="../core/img/ueditor/ueditor.all.min.js"></script>';
        define('XIAOCMS_EDITOR_U', 1);
      }

      if ($setting['toolbar'] == 0) {
        $str .= "<script type='text/javascript'>var ue = UE.getEditor('{$name}', {toolbars: [['bold', 'italic', 'underline', 'inserttable', 'simpleupload', 'insertimage', 'strikethrough', 'superscript', 'subscript', 'removeformat', 'formatmatch', 'autotypeset', 'blockquote', 'pasteplain', '|', 'forecolor', 'backcolor', 'map', 'selectall', 'cleardoc', 'drafts']],serverUrl: '{$up}',});</script>";

      } elseif($setting['toolbar'] == 2) {
        $str .= "<script type='text/javascript'>var ue = UE.getEditor('{$name}', {toolbars: [[" . $items . "]],serverUrl: '{$up}',});</script>";
    } else {
        $str .= "<script type='text/javascript'>var ue = UE.getEditor('{$name}', {serverUrl: '{$up}',});</script>";
      }
    ...
    public static function file($name, $content = '', $setting = '')
    {
        ...
        var uploader = WebUploader.create({
            ...
            server: './index.php?c=uploadfile&a=ueditor&action=uploadfile',
        ...
        return '<input type="text" class="input-text"  size="50" value="' . $content . '" name="data[' . $name . ']" id="' . $name . '" ' . $preview . '><span id="filePicker_' . $name . '"  >'.$txt.'</span>'.$js;
    }
    ...
    public static function files($name, $content = '', $setting = '')
    {
      ...
      <fieldset class="blue pad-10">
        ...
        $str .= '</ul></fieldset>
    <div class="bk10"></div>
    <input type="button"  class="button" value="添加地址" name="delete" onClick="add_null_file(\'' . $name . '\')" > <span id="filePicker_' . $name . '"  >'.$txt.'</span>
    ';
        return $str . $js;
    }
