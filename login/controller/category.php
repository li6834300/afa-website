...
class category extends Admin {
	...
	public function indexAction() {
	    if (Request::post('listorder')) {
     		foreach (Request::post('listorder') as $catid => $value) {
		...
		$this->tree->nbsp = '&nbsp;&nbsp;&nbsp;';
		$categorys = array();
		$cats =  $this->db->setTableName('category')->getAll(null, null, null,'listorder DESC,catid ASC');
		$types = array(1 => '',2 => '<font color="blue">单页面</font>',3 => '<font color="red">外部连接</font>', 4 => '<font color="blue">表单模型</font>');
		if(!empty($cats)) {
			foreach($cats as $r) {
	...
	public function addAction() {
	    ...
	    if (Request::isPost()) {
	        ...
	        if ($data['typeid'] == 1) {
			    ...
			    $data['modelid'] = $data['bdmodelid'];
	            if (empty($data['modelid'])) $this->show_message('请选择绑定表单模型',2,1);
	        } else {
	            $this->show_message('请选择栏目类型',2,1);
	        }

			$data = $this->post_check_fields($category_fields, $data);


	        if (Request::post('addall')) {//批量添加
 	    ...
 	    $data_fields = $this->get_data_fields($category_fields, $data);
		$catdata =  $this->db->setTableName('category')->getAll(null, null, array('catid', 'catname','parentid'),'listorder DESC,catid ASC');
    ...
    public function editAction() {
		...
		if(!empty($catdata)) {
			foreach($catdata as $r) {
			    $r['disabled'] = ($r['catid']==$catid) ? 'disabled' : '';
				$category_select[$r['catid']] = $r;
			}
		}
    ...
    /*
     * [修复栏目关系] 获取顶级栏目数据 免多次查询法 
     */
    private function _getTopid($cat)
    {
        if (empty($cat['parentid'])) {
            return $cat['catid'];
        }
        return $this->_getTopid($this->catsdata[$cat['parentid']]);
    }
	...
	public function cacheAction() {
	    $this->repairAction(); 
		$data =  $this->db->setTableName('category')->getAll(null, null, null,'listorder DESC,catid ASC');
		$this->catsdata = format_array_key($data, 'catid');
	    ...
	    foreach ($data as $t) {
            ...
            $category[$catid]['topid']  = $this->_getTopid($t);
	...
	*/
	private function repairAction() {
		$data = $this->db->setTableName('category')->getAll(null, null, 'catid','listorder DESC,catid ASC');
	    foreach ($data as $t) {
