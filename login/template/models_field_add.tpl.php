...
<div class="subnav">
	...
	<div class="bk10"></div>
		...
		<tr>
			...
			<td><input class="input-text" type="text" id="field" name="data[field]" value="<?php echo $data['field']; ?>" size="30" <?php if ($data[fieldid]) { ?>disabled<?php } ?> /><div class="onShow">必须英文字母开头、数字和下划线组成。如：ceshi</div>
		</tr>
		...
		<tr>
			...
			if ($data['fieldid']) {
				...
				$field = new field();
				if (method_exists($field,$fieldsetting)) echo $field->$fieldsetting($data['setting']);
			} ?>
			</div>
			</td>
		</tr>
