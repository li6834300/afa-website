...
class content extends Admin {
	...
	public function indexAction() {
	    $catid    = (int)Request::get('catid');
		if (Request::post('batch') && Request::post('status')=='del') {
	        foreach (Request::post('batch') as $id) {
	                $this->delAction($id, $catid, 1);
			}
			$this->show_message('删除成功', 1);
	    }elseif (Request::post('batch') && Request::post('status')=='move') {
		    $movecatid = (int)Request::post('movecatid');
	    ...
	    }elseif (Request::post('status') == 'listorder' && Request::post('listorder')) {
     		foreach (Request::post('listorder') as $id => $value) {
	            $this->db->setTableName('content')->update(array('listorder'=>$value), 'id=?' , $id);
			}
			$this->show_message('排序成功', 1);
	    }elseif (Request::post('batch') && !Request::post('move')) {
	    ...
	    $this->db->order(array('listorder DESC', 'time DESC'));
	    $listArr = $this->db->setTableName('content')->page($page, $pagesize);
	...
	public function delAction($id=0,$catid=0,$show=0) {
	    ...
	    $tp = $this->db->setTableName('content')->getAll('id=?' , $id);	//通过id获取缩略图	
    	$filename = '../'.$tp[0][thumb];//文件路径遍历
		if ($this->category_cache[$catid]['tablename']) 
		$this->db->setTableName('content')->delete('id=?' , $id);
		unlink($filename);//删除缩略图
	...
	private function additionalAction($data) {
		...
		$bt = str_replace('&quot;', '',$data['title']);
		$data['content'] = str_replace('alt=&quot;&quot;', 'alt=&quot;'.$bt.'&quot;',$data['content']);
		return $data;
	}
