...
<div class="subnav">
  <div class="content-menu"> <a href="<?php echo url('block'); ?>"  class="on"><em>全部区块</em></a>
  ...
  <form action="" method="post">
    ...
    <table width="100%" class="table_form">
      ...
      <tr>
        <th width="80">区块名称： </th>
        <td><input class="input-text" type="text" name="data[name]" value="<?php echo $data['name']; ?>" size="40"/>
          编辑方式：
          <select class="select" id="type" name="data[type]" onChange="select_type(this.value)">
      ...
      <?php } else {?>
      <tr>
        <th width="80"><input name="data[name]" type="hidden" value="<?php echo $data['name']; ?>" /></th>
        <td><?php if (is_array($this->type))  foreach ($this->type as $i=>$v) { ?>
          <?php if ($data['type']==$i) { ?>
          <input name="data[type]" type="hidden" value="<?php echo $i; ?>" />
          <?php } ?>
          <?php }  ?></td>
      </tr>
      <?php }  ?>
      <tr id="text_1" style="display:none">
        ...
        <td><textarea name="data[content_1]" id="data[content]" cols="105" rows="28" style="width: 680px; height: 385px;"><?php echo $data['content']; ?></textarea>
          <br>
          <div class="onShow">区块内容支持HTML标签</div></td>
      </tr>
      <tr id="text_2" style="display:none">
        <th>区块内容： </th>
        <td><?php $field = new field();  echo $field->file('content_2',$data['content'],array('type'=>'gif,jpg,jpeg,png','preview'=>1,'size'=>2,'buttontxt'=>'上传图片')); ?></td>
      </tr>
      <tr id="text_3" style="display:none;">
        <th>区块内容：</th>
        <td><?php echo $field->editor('content_3', $data['content'], array('system'=>1,'toolbar'=>1)); ?></td>
      </tr>
      <tr>
        <th>&nbsp;</th>
